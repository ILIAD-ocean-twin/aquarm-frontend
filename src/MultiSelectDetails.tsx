...
export const MultiSelectDetails: Component<MultiSelectDetailsProps> = (props) => {
    ...
    createEffect(() => {
        const locs = props.sites.map(s => s.id).join(",")
        fetch(`/lice?localities=${locs}&from_year=${props.time.year - 1}&from_week=${props.time.week}&to_year=${props.time.year}&to_week=${props.time.week}`)
            .then(resp => resp.json())
            .then(data => {
