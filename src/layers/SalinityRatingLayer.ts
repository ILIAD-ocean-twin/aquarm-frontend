...
export class SalinityRatingLayer implements IDataLayer {
  ...
  constructor() {
    ...
    fetch("http://localhost:8800/metadata/salinity")
      .then(resp => resp.json())
      .then(meta => {
        const url = "http://localhost:8800/singleband/salinity/{z}/{x}/{y}.png?colormap=spectral&stretch_range=[" + (meta.range[0]) + "," + (meta.range[1]) + "]";
